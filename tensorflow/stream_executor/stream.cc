...
Stream &Stream::ThenConvolveWithExecutionPlan(
    const dnn::BatchDescriptor &input_descriptor,
    const DeviceMemory<double> &input_data,
    const dnn::FilterDescriptor &filter_descriptor,
    const DeviceMemory<double> &filter_data,
  ...
  VLOG_CALL(PARAM(input_descriptor), PARAM(input_data),
            PARAM(filter_descriptor), PARAM(filter_data),
            PARAM(convolution_descriptor), PARAM(output_descriptor),
            PARAM(output), PARAM(plan_config));

  if (ok()) {
    if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
      auto status = dnn->DoConvolve(
          this, input_descriptor, input_data, filter_descriptor, filter_data,
          convolution_descriptor, output_descriptor, output, plan_config,
...
Stream &Stream::ThenConvolveBackwardDataWithExecutionPlan(
    const dnn::FilterDescriptor &filter_descriptor,
    const DeviceMemory<double> &filter_data,
    const dnn::BatchDescriptor &output_descriptor,
    DeviceMemory<double> backward_output_data,
  ...
  if (ok()) {
    if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
      auto status = dnn->DoConvolveBackwardData(
          this, filter_descriptor, filter_data, output_descriptor,
          backward_output_data, convolution_descriptor, input_descriptor,
          backward_input_data, plan_config, scratch_allocator,
          output_profile_result);
      if (!status && !output_profile_result) {
        SetError();
      }
...
Stream &Stream::ThenConvolveBackwardDataWithExecutionPlan(
  ...
  VLOG_CALL(PARAM(filter_descriptor), PARAM(filter_data),
            PARAM(output_descriptor), PARAM(backward_output_data),
            PARAM(convolution_descriptor), PARAM(input_descriptor),
            PARAM(backward_input_data), PARAM(plan_config));
...
Stream &Stream::ThenConvolveBackwardFilterWithExecutionPlan(
    const dnn::BatchDescriptor &input_descriptor,
    const DeviceMemory<double> &input_data,
    const dnn::BatchDescriptor &output_descriptor,
    DeviceMemory<double> backward_output_data,
  ...
  if (ok()) {
    if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
      auto status = dnn->DoConvolveBackwardFilter(
          this, input_descriptor, input_data, output_descriptor,
          backward_output_data, convolution_descriptor, filter_descriptor,
          backward_filter_data, plan_config, scratch_allocator,
          output_profile_result);
      if (!status && !output_profile_result) {
        SetError();
      }
...
Stream& Stream::ThenBlasGemmStridedBatchedWithAlgorithm(
    blas::Transpose transa, blas::Transpose transb, uint64 m, uint64 n,
    uint64 k, double alpha, const DeviceMemory<Eigen::half>& a, int lda,
    int64 stride_a, const DeviceMemory<Eigen::half>& b, int ldb, int64 stride_b,
    double beta, DeviceMemory<Eigen::half>* c, int ldc, int64 stride_c,
    int batch_count, blas::ComputationType computation_type,
    blas::AlgorithmType algorithm, blas::ProfileResult* output_profile_result) {
  VLOG_CALL(PARAM(transa), PARAM(transb), PARAM(m), PARAM(n), PARAM(k),
            PARAM(alpha), PARAM(a), PARAM(lda), PARAM(stride_a), PARAM(b),
            PARAM(ldb), PARAM(stride_b), PARAM(beta), PARAM(c), PARAM(ldc),
            PARAM(stride_c), PARAM(batch_count), PARAM(computation_type),
            PARAM(algorithm));

  ThenBlasWithProfileImpl<blas::Transpose, blas::Transpose, uint64, uint64,
                          uint64, double, const DeviceMemory<Eigen::half>&, int,
                          int64, const DeviceMemory<Eigen::half>&, int, int64,
                          double, DeviceMemory<Eigen::half>*, int, int64, int,
                          blas::ComputationType, blas::AlgorithmType>
      impl;
  return impl(this, &blas::BlasSupport::DoBlasGemmStridedBatchedWithAlgorithm,
...
Stream &Stream::ThenBlasGemmStridedBatchedWithAlgorithm(
  ...
  ThenBlasWithProfileImpl<blas::Transpose, blas::Transpose, uint64, uint64,
                          uint64, double, const DeviceMemory<float>&, int,
                          int64, const DeviceMemory<float>&, int, int64, double,
                          DeviceMemory<float>*, int, int64, int,
                          blas::ComputationType, blas::AlgorithmType>
      impl;
  return impl(this, &blas::BlasSupport::DoBlasGemmStridedBatchedWithAlgorithm,
...
Stream &Stream::ThenBlasGemmStridedBatchedWithAlgorithm(
  ...
  ThenBlasWithProfileImpl<blas::Transpose, blas::Transpose, uint64, uint64,
                          uint64, double, const DeviceMemory<double>&, int,
                          int64, const DeviceMemory<double>&, int, int64, double,
                          DeviceMemory<double>*, int, int64, int,
                          blas::ComputationType, blas::AlgorithmType>
      impl;
  return impl(this, &blas::BlasSupport::DoBlasGemmStridedBatchedWithAlgorithm,
...
Stream& Stream::ThenBlasGemmStridedBatchedWithAlgorithm(
  ...
  ThenBlasWithProfileImpl<
      blas::Transpose, blas::Transpose, uint64, uint64, uint64,
      std::complex<float>, const DeviceMemory<std::complex<float>>&, int, int64,
      const DeviceMemory<std::complex<float>>&, int, int64, double,
      DeviceMemory<std::complex<float>>*, int, int64, int,
      blas::ComputationType, blas::AlgorithmType>
      impl;
  return impl(this, &blas::BlasSupport::DoBlasGemmStridedBatchedWithAlgorithm,
              transa, transb, m, n, k, alpha, a, lda, stride_a, b, ldb,
...
Stream& Stream::ThenBlasGemmStridedBatchedWithAlgorithm(
  ...
  ThenBlasWithProfileImpl<
      blas::Transpose, blas::Transpose, uint64, uint64, uint64,
      std::complex<double>, const DeviceMemory<std::complex<double>>&, int,
      int64, const DeviceMemory<std::complex<double>>&, int, int64, double,
      DeviceMemory<std::complex<double>>*, int, int64, int,
      blas::ComputationType, blas::AlgorithmType>
      impl;
  return impl(this, &blas::BlasSupport::DoBlasGemmStridedBatchedWithAlgorithm,
